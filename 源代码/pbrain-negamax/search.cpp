...
int alphabeta(int depth,int alpha,int beta,int player,int MaxDepth)
{
	...
	for (int i = 0; i<moveListLen; i++)
	{
		...
		bestVal = -alphabeta(depth-1,-beta,-alpha,1-player,MaxDepth);
		UnmakeMove(moveList[i]);
		//未改进的alpha-beta可以直接采用下面这段代码
		...
		if (bestVal<beta)
		{
			if (bestVal>alpha)
			{
				...
				MakeMove(moveList[i], 1 - player);
				int value = -alphabeta(depth-1,-beta,-alpha,1-player,MaxDepth);
